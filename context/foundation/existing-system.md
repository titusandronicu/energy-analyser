...
|---|---|---|
| **Home Assistant** | Live, production | Live PV / battery / grid / load telemetry. Deye inverter integrated via DeyeCloud (primary since 2026-07-21; Solarman local kept for comparison only). HA conversation agent. The solar forecast integration was not recreated when HA moved hosts (~2026-07-21); see *Lab state (2026-09-25)*. |
...
| **History storage** | Live | Private SQLite: multi-month HA/Deye snapshot history plus PGE hourly readings (checked 2026-09-26: the lab's `energy-history.jsonl` starts on 2026-07-16; PGE data covers February to July 2026, grid import/export only). Public-safe JSON/JSONL cache (~90 days) and Markdown memory files for the UI and agents. |
...
| LLM narration (FR-005) | Provider chain, Polish output | Recorded 2026-09-23 as HA → Ollama → OpenRouter; the live order (checked 2026-09-26) is OpenRouter → Ollama → HA, and all recommendations so far came from OpenRouter. See docs/prerequisites.md |
...
- Notes on days CRUD (FR-025–028; replaced recommendation feedback, FR-007–010, in PRD v3).
...
| The push step (homelab-2 change) | Ingestion endpoint + payload contract; notes on days CRUD (FR-025–028, formerly feedback FR-007–010); day and month ratings (FR-022) |
