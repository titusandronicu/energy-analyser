...
| **Forecast.Solar** (core) | HA UI | Comparison forecast (`sensor.energy_production_*`) | S-11 source comparison | One plane: declination 0, azimuth 0 (HA: 0 = north), modules power 10000 W, no API key. Comparison only; removing it requires removing its four keys from `ENTITY_MAP`. |
...
| S-11 accuracy and certainty | 1–2 weeks of Solcast forecasts after F-05; low/high estimates are already recorded in lab history. |
| S-12 inverter schedule | Lab Deye settings snapshot (`collect-deye-settings-snapshot.py`). |
| S-16 year view | F-03 backfill. |
| S-18 summaries | F-04. |
